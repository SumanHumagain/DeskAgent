...
class WindowsGUIAutomation:
    ...
    def find_window(self, search_terms: List[str], timeout: int = 5, debug: bool = True) -> Optional[Any]:
                ...
                for window in windows:
                    try:
                        ...
                        for term in search_terms:
                            # Allow empty term to match any window (fallback)
                            if not term or term.lower() in title.lower():
                                print(f"[GUI CORE] Found window: '{title}'", file=sys.stderr)
                                return window
        ...
        if debug and all_window_titles:
            ...
            for title in sorted(all_window_titles)[:20]:  # Show first 20
