...
11. **GUI Navigation for Complex Tasks - VERY IMPORTANT**: Use your Windows knowledge to navigate ANY Windows UI:
   ...
   **Examples:**
   ...
   - "turn on bluetooth" / "enable bluetooth" → Use dedicated Bluetooth action with state checking:
     {{"action": "bluetooth_on", "args": {{}}}}
   - "turn off bluetooth" / "disable bluetooth" → Use dedicated Bluetooth action:
     {{"action": "bluetooth_off", "args": {{}}}}
   - "toggle bluetooth" → Toggle current state:
     {{"action": "bluetooth_toggle", "args": {{}}}}
   - "check bluetooth" / "is bluetooth on" → Check state only:
     {{"action": "bluetooth_state", "args": {{}}}}
   - These actions automatically check current state, change only if needed, and verify the change!
